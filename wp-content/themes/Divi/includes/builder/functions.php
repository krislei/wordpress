...
function et_builder_enqueue_font( $font_name ) {
    ...
    if($font_name_slug=='et-gf-microsoft-yahei'||$et_gf_font_name_slug=='et-gf-simsun'||$et_gf_font_name_slug=='et-gf-simhei'||$et_gf_font_name_slug=='et-gf-fangsong'||$et_gf_font_name_slug=='et-gf-kaiti'){
			echo '';
		}else{
	wp_enqueue_style( $font_name_slug, esc_url( add_query_arg( $query_args, "http://fonts.googleapis.com/css" ) ), array(), null );
	}
}
endif;
...
function et_pb_add_builder_page_js_css(){
	...
	wp_enqueue_script( 'google-maps-api', esc_url( add_query_arg( array( 'v' => 3, 'sensor' => 'false' ), is_ssl() ? 'https://maps-api-ssl.google.com/maps/api/js' : 'http://www.google.cn/maps/api/js' ) ), array(), '3', true );
...
function et_get_first_video() {
    ...
    preg_match('#https?://v\.youku\.com/v_show/id_(?<video_id>[a-z0-9_=\-]+)#i', get_the_content(), $match);
	//$youku='http://v.youku.com/v_show/id_'.$match[1].'.html';
	if(''!==$match[1]){
		if (wp_is_mobile()) {
			$height='300px';
			}else{
    ...
    //$djurl=get_bloginfo('template_url');
	//$xml_array=simplexml_load_file("$djurl/ckplayer/video.php?url='.$youku.'");
	//foreach($xml_array as $tmp){
	//$url5=$tmp->file;
	//}
	//$player_id = 'a' . rand( 10, 400000 );
	//echo '<div id="'.$player_id.'"></div>
	//			<script type="text/javascript" src="'.get_bloginfo('template_url').'/ckplayer/ckplayer.js" charset="utf-8"></script>
	//			<script type="text/javascript">
	//				var flashvars={
	//				   f:"'.get_bloginfo('template_url').'/ckplayer/video.php?url=[\$pat]",
	//					a:"'.$youku.'",
	//					p:0,
	//					s:"2"	
	//				};
	//				var params={bgcolor:"#FFF",allowFullScreen:true,allowScriptAccess:"always",wmode:"transparent"};
	//				var video=["'.$url5.'->video/mp4"];
	//				CKobject.embed("'.get_bloginfo('template_url').'/ckplayer/ckplayer.swf","'.$player_id.'","ckplayer_'.$player_id.'","100%","80%",false,flashvars,video,params);					
	//			</script>';
	echo '<iframe src="http://player.youku.com/embed/' . $match[1] . '" width="100%" height="'.$height.'" frameborder="0" allowfullscreen="true"></iframe>';
	}else{
	...
	printf(
		'<div class="et_pb_roles_main_container">
			...
			<div id="et_pb_main_container" class="post-type-page">
				<div id="et_pb_layout_controls">
					%1$s
					<a href="#" class="et-pb-layout-buttons et-pb-layout-buttons-reset" title="重置所有设置">
						<span class="icon"></span><span class="label">重置</span>
					</a>
				</div>
			</div>
