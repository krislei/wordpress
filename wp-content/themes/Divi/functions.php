...
function wp_embed_handler_youku2( $matches, $attr, $url, $rawattr ) {	
	$embed ='';
	return apply_filters( 'embed_youku2', $embed, $matches, $attr, $url, $rawattr );
}
wp_embed_register_handler( 'youku2',
...
function my_login_redirect($redirect_to, $request){
  if( empty( $redirect_to ) || $redirect_to == 'http://103.251.88.196/olscomsys/' || $redirect_to == admin_url() )
    return home_url();
  else
    return $redirect_to;
}
